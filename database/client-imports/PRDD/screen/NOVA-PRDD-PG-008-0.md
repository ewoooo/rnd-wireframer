---
화면 ID: NOVA-PRDD-PG-008-0
화면 명: 가입·구매 조건 검증
화면 설명: 채널이 선택 구성의 가입 가능성, 중복 보유, 선행 조건, 판매 가능 상태를 검증한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | 가입 조건 판정 결과와 조합 검증 결과 기본 표시 | apiCall |
...
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PG-009-0 | 예상 비용·혜택 요약 | 가입·구매 조건 검증 결과가 성공으로 확정된 경우 | 선택 구성, 판정 근거 | - |
| 케이스 분기 | NOVA-PRDD-PG-008-1 | 가입·구매 조건 검증-재고·판매 상태·가입 조건 미확정 | 재고·판매 상태·가입 조건 미확정 | - | 담기 미확정 후 보완 항목 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-008-2 | 가입·구매 조건 검증-선택 조합 충돌 | 선택 조합 충돌 | - | 충돌 항목만 수정 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-008-3 | 가입·구매 조건 검증-로그인·인증 후 복귀 시 선택 구성 소실 | 로그인·인증 후 복귀 시 선택 구성 소실 | - | 선택 구성 복원 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-008-4 | 가입·구매 조건 검증-재고·판매 상태·가입 조건 미확정 | 재고·판매 상태·가입 조건 미확정 | - | 담기 미확정 후 보완 항목 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-008-5 | 가입·구매 조건 검증-선택 조합 충돌 | 선택 조합 충돌 | - | 충돌 항목만 수정 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-008-6 | 가입·구매 조건 검증-로그인·인증 후 복귀 시 선택 구성 소실 | 로그인·인증 후 복귀 시 선택 구성 소실 | - | 선택 구성 복원 후 화면 유지 |
