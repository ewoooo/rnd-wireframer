---
화면 ID: NOVA-PRDD-PU-016-E5
화면 명: 로그인·인증 후 원위치 복귀-미디어·후기·스펙 로딩 실패
화면 설명: 미디어·후기·스펙 로딩 실패 — 핵심 요약과 가격·조건 판단은 유지
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | [영역 2] 미디어·후기·스펙 영역 로딩 실패 표시 + 핵심 요약·가격·조건 유지 | apiCall |
...
| 999 | 1 | ActionButtonRecover | 화면 하단 인증 진행 CTA | ActionButton | default | - | - | - | main: 계속하기 | 원위치 복귀 정보 (api:FN-PRDD-AUTH-001) | [정책:PI-PRDD-FAIL-001-03] 인증 복귀 |
