---
화면 ID: NOVA-PRDD-PU-016-0
화면 명: 로그인·인증 후 원위치 복귀
화면 설명: 로그인, 회선 확인, 추가 인증이 필요한 이유를 안내하고 완료 후 기존 선택 상태로 복귀한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | 인증 필요 사유와 선택 유지 안내 기본 표시 | apiCall |
| loading | API 호출 | skeleton 표시 | - |
| error | 인증 실패 후 원위치 복귀 불가 | [영역 1] Callout 오류 안내 + 다시 선택 항목 강조 | apiCall |
| empty | 앱 설치 완료 상태 | [영역 2] 미설치 유도 안내 미노출 | - |
| blocked | 본인확인 미완료 | [영역 1] 인증 안내 강조 + CTA disabled | setState |
...
| 999 | 1 | ActionButtonRecover | 화면 하단 인증 진행 CTA | ActionButton | default | onClick | navigate | NOVA-PRDD-PG-017-0 | main: 인증 완료하고 계속하기 | 원위치 복귀 정보 (api:FN-PRDD-AUTH-001) | [정책:PI-PRDD-FAIL-001-03] 인증 복귀 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PG-017-0 | 상담 전환과 실패 이력 전달 | 로그인·인증 후 원위치 복귀 결과 확정 시 | 원위치 복귀 토큰, 인증 결과, 이전 선택 구성 | - |
| 케이스 분기 | NOVA-PRDD-PU-016-E1 | 로그인·인증 후 원위치 복귀-동일 실패 반복 | 동일 실패 반복 | - | 대체 상품·상담·나중에 다시 시도 경로 제시 |
| 케이스 분기 | NOVA-PRDD-PU-016-E2 | 로그인·인증 후 원위치 복귀-상담 전환 문맥 누락 | 상담 전환 문맥 누락 | - | 상품 ID·옵션·오류 사유 보완 후 상담 접수 |
| 케이스 분기 | NOVA-PRDD-PU-016-E3 | 로그인·인증 후 원위치 복귀-인증 실패 후 원위치 복귀 불가 | 인증 실패 후 원위치 복귀 불가 | - | 다시 선택해야 하는 항목 명확히 안내 |
| 케이스 분기 | NOVA-PRDD-PU-016-E4 | 로그인·인증 후 원위치 복귀-상품 기준 정보 누락 | 상품 기준 정보 누락 | - | 섹션 숨김 없이 보완 필요·상담 가능 경로 안내 |
| 케이스 분기 | NOVA-PRDD-PU-016-E5 | 로그인·인증 후 원위치 복귀-미디어·후기·스펙 로딩 실패 | 미디어·후기·스펙 로딩 실패 | - | 핵심 요약과 가격·조건 판단 유지 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PU-016-1 | 로그인·인증 후 원위치 복귀-내부 운영 코드·원장 필드명 고객 노출 | 내부 운영 코드·원장 필드명 고객 노출 | - | 배포 제한 |
