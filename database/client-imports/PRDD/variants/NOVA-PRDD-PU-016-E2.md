---
화면 ID: NOVA-PRDD-PU-016-E2
화면 명: 로그인·인증 후 원위치 복귀-상담 전환 문맥 누락
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | [영역 1] Callout 보완 안내 + 누락 항목 강조 | - |
...
| 999 | 1 | ActionButtonRecover | 화면 하단 인증 진행 CTA | ActionButton | default | - | - | - | main: 보완하고 상담 접수 | 원위치 복귀 정보 (api:FN-PRDD-AUTH-001) | [정책:PI-PRDD-FAIL-001-03] 인증 복귀 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PU-016-0 | 로그인·인증 후 원위치 복귀 | 상담 전환 문맥 누락 해소 후 | - | - |
