---
화면 ID: NOVA-PRDD-PU-016-E4
화면 명: 로그인·인증 후 원위치 복귀-상품 기준 정보 누락
화면 설명: 상품 기준 정보 누락 — 해당 섹션을 숨기지 않고 보완 필요 또는 상담 가능 경로 안내
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | [영역 1] 누락 섹션에 보완 필요 안내 표시 + 상담 가능 경로 강조 | - |
...
| 999 | 1 | ActionButtonRecover | 화면 하단 인증 진행 CTA | ActionButton | default | - | - | - | main: 보완 정보 확인하기 | 원위치 복귀 정보 (api:FN-PRDD-AUTH-001) | [정책:PI-PRDD-FAIL-001-03] 인증 복귀 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PU-016-0 | 로그인·인증 후 원위치 복귀 | 상품 기준 정보 누락 해소 후 | - | - |
