---
화면 ID: NOVA-PRDD-PG-010-E2
화면 명: 공유·딥링크·문의 맥락 유지-동일 실패 반복
화면 설명: 동일 실패 반복 (반복 실패) — 재시도만 제공하지 않고 대체 상품, 상담, 나중에 다시 시도 경로를 제시한다
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 시 동일 실패 반복 상태 | 재시도만 제공하지 않고 대체 상품, 상담, 나중에 다시 시도 경로를 제시한다 — 보완 항목 강조 + Callout 안내 노출 | setState |
...
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PG-010-0 | 공유·딥링크·문의 맥락 유지 | 동일 실패 반복 해소 후 | - | - |
