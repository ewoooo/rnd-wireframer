---
화면 ID: NOVA-PRDD-PG-010-E3
화면 명: 공유·딥링크·문의 맥락 유지-인증 실패 후 원위치 복귀 불가
화면 설명: 인증 실패 후 원위치 복귀 불가 (인증 실패) — 다시 선택해야 하는 항목을 명확히 안내한다
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 시 인증 실패 후 원위치 복귀 불가 상태 | 핵심 요약·가격·조건 판단은 유지하고 Callout 오류 안내 노출 | apiCall |
...
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PG-010-0 | 공유·딥링크·문의 맥락 유지 | 인증 실패 후 원위치 복귀 불가 해소 후 | - | - |
