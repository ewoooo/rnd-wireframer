---
화면 ID: NOVA-PRDD-PG-015-2
화면 명: 선택 불가·충돌 사유 안내-로그인·인증 후 복귀 시 선택 구성 소실
화면 설명: 로그인·인증 후 복귀 시 선택 구성 소실 — 재선택 없이 복원 가능한 상태로 전환
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | [영역 2] 복원된 선택 구성 표시 + 재선택 없이 복원 안내 | apiCall |
