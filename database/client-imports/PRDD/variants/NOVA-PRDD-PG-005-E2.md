---
화면 ID: NOVA-PRDD-PG-005-E2
화면 명: 비교 컴포넌트로 대안 비교-미디어·후기·스펙 로딩 실패
화면 설명: 미디어·후기·스펙 로딩이 실패해도 핵심 요약과 가격·조건 판단은 유지한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 화면 진입 정상 (미디어·후기·스펙 로딩 실패 상태) | [영역 1] Callout 오류 안내 노출, 핵심 요약·가격 판단은 유지 | apiCall |
| OneButton | 로딩 실패 안내 확인 후 CTA 1개 활성 | [영역 999] ActionButton main 슬롯 활성 | - |
...
| 1 | 5 | CardContentsFilledPlanCompare | 요금제 비교표 (로딩 실패 안내 노출) | CardContentsFilled | - | - | - | - | titleText: 요금제 비교 — 일부 항목 불러오기 실패 | 상품군별 필수 정보 표시 여부 (api:FN-PRDD-COMPARE-001) | [정책:PI-PRDD-COMPARE-001-06] 상품군별 비교 템플릿 |
| 2 | 1 | TitleSectionPrice | 예상 부담·혜택 요약 영역 제목 | TitleSection | Default | - | - | - | title: 예상 부담·혜택 요약 | - (static) | - |
| 2 | 2 | CardSummaryMonthlyEstimate | 월 예상액 요약 (핵심 요약 유지) | CardSummary | text | - | - | - | title: 월 예상액<br>subText: {월 예상 금액} (예: 월 55,000원) | 고객용 상품 요약 (api:FN-PRDD-PRICE-001)<br>월 기준 금액 (policy:PI-PRDD-PRICE-001-01) | [정책:PI-PRDD-PRICE-001-01] 가격 기준 |
| 2 | 3 | ListTextOneTimeCost | 1회성 비용 표시 (가격 판단 유지) | ListText | on | - | - | - | title: 1회성 비용<br>subText: {1회성 비용 금액} (예: 12,000원) | 고객용 상품 요약 (api:FN-PRDD-PRICE-001)<br>1회성 비용 (policy:PI-PRDD-PRICE-001-01) | [정책:PI-PRDD-PRICE-001-01] 가격 기준 |
...
| 2 | 5 | AccordionPriceInfoSubsidy | 공시지원금 비교 펼침 (가격 판단 유지) | AccordionPriceInfo | default | onClick | setState | isSubsidyAccordionOpen | priceText: 공시지원금·선택약정 비교 | 출고가·공시지원금·선택약정 적용가 (api:FN-PRDD-PRICE-001)<br>공시지원금 고지 기준 (policy:PI-PRDD-PRICE-001-03) | [정책:PI-PRDD-PRICE-001-03] 공시지원금 |
| 999 | 1 | ActionButton | 화면 하단 진행 CTA | ActionButton | default | onClick | navigate | main: NOVA-PRDD-PG-005-0 | main: 다시 불러오기 | - (static) | [정책:PI-PRDD-PRICE-001-01] 가격 기준 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-005-0 | 비교 컴포넌트로 대안 비교 | 미디어·후기·스펙 로딩 실패 해소 후 | - | - |
