---
화면 ID: NOVA-PRDD-PG-011-2
화면 명: 담기 전 유효성 재검증-동일 구성 중복 담기
화면 설명: 동일 구성이 이미 담겨 있어 중복 저장하지 않고 기존 담기 항목 갱신 또는 이동 경로를 안내한다.
화면 경로: 담기 전 유효성 재검증
구현 유형: PG
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 동일 구성 중복 담기 감지 | [영역 1] Callout 기존 담기 항목 갱신·이동 안내 노출 | - |
| OneButton | 기존 담기 항목 갱신·이동 안내 확인 | ActionButton main 슬롯만 활성 | - |
...
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-011-0 | 담기 전 유효성 재검증 | 동일 구성 중복 담기 해소 후 | - | - |
