---
화면 ID: NOVA-PRDD-PG-014-E2
화면 명: 장바구니·주문 진입 연결-재검증 실패
화면 설명: 진입 전 재검증이 실패해 담기 완료 상태는 유지하되 주문 전환은 제한한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 진입 전 재검증 실패 | [영역 999] 주문 전환 CTA disabled + 담기 완료 상태 유지 | setState |
| TwoButton | 진입 전 재검증 통과 후 진입 구성 확정 | ActionButton main·secondary 슬롯 활성 | - |
...
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-014-0 | 장바구니·주문 진입 연결 | 재검증 실패 해소 후 | - | - |
