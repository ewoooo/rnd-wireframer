---
화면 ID: NOVA-PRDD-PG-006-2
화면 명: 추천·AI 요약 근거 검토-후기 필터·탭 결과 없음
화면 설명: 후기 필터·탭 결과가 없으면 조건 완화, 전체 후기 보기, 후기 작성 가능 여부 중 하나 이상을 안내한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 화면 진입 정상 (후기 필터·탭 결과 없음 상태) | [영역 2] 필터 완화 안내 Callout 노출, 후기 목록 빈 상태 | - |
...
| 1 | 4 | CardTextReviewSummary | 구매후기 AI 요약 | CardText | Default | - | - | - | title: 후기 AI 요약<br>body: 표시할 후기 요약이 없습니다 — 조건을 완화하면 후기를 다시 볼 수 있습니다 | 고객용 상품 요약 (api:FN-PRDD-AI-001) | [정책:PI-PRDD-COMPARE-001-04] 후기 요약 |
...
| 2 | 2 | InfoTextListReview | 후기 목록 (결과 없음 빈 상태) | InfoTextList | Default | - | - | - | title: 조건에 맞는 후기가 없습니다 | 상품 상세 조회와 비교·담기 전환 이력 (api:FN-PRDD-REVIEW-001) | [정책:PI-PRDD-REVIEW-001-04] 결과 없음 |
...
| 2 | 5 | CalloutFilterRelaxGuide | 필터 완화 안내 | Callout | WithTitle | onClick | setState | reviewFilterReset | title: 조건에 맞는 후기가 없습니다<br>body: 조건 완화, 전체 후기 보기, 후기 작성 가능 여부 중 하나 이상을 안내합니다. | reviewFilterReset (state)<br>결과 없음 안내 기준 (policy:PI-PRDD-REVIEW-001-04) | [정책:PI-PRDD-REVIEW-001-04] 결과 없음 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-006-0 | 추천·AI 요약 근거 검토 | 후기 필터·탭 결과 없음 해소 후 | - | - |
