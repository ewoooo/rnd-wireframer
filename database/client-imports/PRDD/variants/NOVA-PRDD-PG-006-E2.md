---
화면 ID: NOVA-PRDD-PG-006-E2
화면 명: 추천·AI 요약 근거 검토-미디어·후기·스펙 로딩 실패
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 화면 진입 정상 (미디어·후기·스펙 로딩 실패 상태) | [영역 2] Callout 오류 안내 노출, 핵심 요약·가격 판단은 유지 | apiCall |
...
| 1 | 2 | CalloutAiSummary | 상품 핵심 특징 AI 요약 (핵심 요약 유지) | Callout | WithTitle | - | - | - | title: AI 핵심 요약<br>body: {AI 요약 문구} — 생성 기준·반영 시점·원문 이동 경로를 함께 표시합니다 | 고객용 상품 요약 (api:FN-PRDD-AI-001)<br>AI 요약 생성 기준 (policy:PI-PRDD-COMPARE-001-02) | [정책:PI-PRDD-COMPARE-001-02] AI 요약 |
...
| 1 | 4 | CardTextReviewSummary | 구매후기 AI 요약 (로딩 실패 안내) | CardText | Default | - | - | - | title: 후기 AI 요약<br>body: 후기 정보를 불러오지 못했습니다 — 핵심 요약과 가격·조건 판단은 그대로 이용할 수 있습니다 | 미노출·대체 안내 사유 (api:FN-PRDD-AI-001) | [정책:PI-PRDD-COMPARE-001-04] 후기 요약 |
...
| 2 | 2 | InfoTextListReview | 후기 목록 (로딩 실패 빈 상태) | InfoTextList | Default | - | - | - | title: 후기를 불러오지 못했습니다 | 상품 상세 조회와 비교·담기 전환 이력 (api:FN-PRDD-REVIEW-001) | [정책:PI-PRDD-REVIEW-001-01] 공통 컬럼 |
...
| 2 | 5 | CalloutFilterRelaxGuide | 필터 완화 안내 | Callout | WithTitle | - | - | - | title: 후기를 불러오지 못했습니다<br>body: 잠시 후 다시 시도하거나 전체 후기 보기로 이동할 수 있습니다. | 미노출·대체 안내 사유 (api:FN-PRDD-REVIEW-001)<br>결과 없음 안내 기준 (policy:PI-PRDD-REVIEW-001-04) | [정책:PI-PRDD-REVIEW-001-04] 결과 없음 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-006-0 | 추천·AI 요약 근거 검토 | 미디어·후기·스펙 로딩 실패 해소 후 | - | - |
