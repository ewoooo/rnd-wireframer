...
src/public/catalog.ts public store loader, lookup, preset, and summaries API
...
Public consumers should import from `@cx/layout-pattern-store`, `@cx/layout-pattern-store/resolver`, `@cx/layout-pattern-store/mutations`, or `@cx/layout-pattern-store/types`. Files under `src/internal/` are implementation details and must not be imported by other packages.
