...
import type { RenderTreeNodeKind } from "../tree/runtime";
import { NODE_TYPES } from "../tree/types";

const structuralNodeKinds = {
	ActionArea: "action",
	"action-area": "action",
} as const satisfies Record<string, RenderTreeNodeKind>;

export function createNodeKindMap() {
	const kindByType = new Map<string, RenderTreeNodeKind>(
		Object.entries(structuralNodeKinds) as Array<[string, RenderTreeNodeKind]>,
	);
