...
describe("screen intent node", () => {
	it("builds the screen-intent agent input and runs the provided agent runner", async () => {
		...
		expect(result.agentInput.context.constraints.sourceSpec).toBe(sourceSpec);
		expect(result.agentInput.context.constraints.targetArtifact.kind).toBe("screen-intent");
