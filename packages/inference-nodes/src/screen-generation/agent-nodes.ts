...
export function runCompositionPlanNode(
	...
	const agentInput = buildCompositionPlanAgentInput(input);
...
export function runPatternSelectionNode(
	...
	const agentInput = buildPatternSelectionAgentInput(input);
	return runAgentPromptNode({
		agentInput,
		runner: input.runner,
...
export function runScreenGenerationNode(
	...
	const agentInput = buildScreenGenerationAgentInput(input.sourceSpec, input);
	return runAgentPromptNode({
		agentInput,
		runner: input.runner,
...
export function runComponentProposalNode(
	...
	const agentInput = buildComponentProposalAgentInput(input);
	return runAgentPromptNode({
		agentInput,
		runner: input.runner,
...
export function runQualityReviewNode(
	...
	// buildQualityReviewAgentInput requires candidate, but the node accepts it optionally,
	// so forward fields explicitly rather than passing the whole input through.
