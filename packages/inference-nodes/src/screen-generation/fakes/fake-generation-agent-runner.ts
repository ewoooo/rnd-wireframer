...
function createFakeRenderTree(agentInput: ScreenGenerationAgentInput) {
	const { screenCode, screenName } = agentInput.context.guidance.sourceSummary;
	...
	return {
		...
		theme: { mode: "light" },
		version: agentInput.context.constraints.targetArtifact.schemaVersion,
	};
}
...
function createFakeTableGenerationResult(agentInput: ScreenGenerationAgentInput) {
	...
	return {
		schemaVersion: agentInput.context.constraints.intermediateArtifact.schemaVersion,
		screen: {
			id: screenCode,
			version: "0.1.0",
...
function resolveSourceRefsByRegion(agentInput: ScreenGenerationAgentInput): {
	...
	const sections = readCompositionPlanSections(agentInput.context.upstream.compositionPlan);
