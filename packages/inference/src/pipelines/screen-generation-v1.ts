import type { PipelineDefinition, StepConstraint } from "../contracts";
import {
	contexts,
	definePipeline,
	defineStep,
...
/**
 * screen-generation 파이프라인 빌더 — strict/enriched 두 변형을 한 정의에서 만든다.
 *
 * - strict(기본, `screen-generation@v1`): 모든 step이 source 충실, 카디널리티 보존.
 *   기존 동작과 100% 동일하다.
 * - enriched(`screen-generation-enriched@v1`): render-tree를 생성/수정하는 claude
 *   step(04/06/09)만 `constraint: "free"`. composition-plan.catalogGaps가 지목한
 *   reference 패턴 컴포넌트로 여러 source 항목을 통합(merge)할 수 있다 — 단 정보
 *   보존은 deterministic 재검증이 보장한다. ux-improvement를 화면에 닫는 경로.
 *
 ...
 */
export function buildScreenGenerationPipeline(options: {
	id: string;
	enrichment: boolean;
}): PipelineDefinition {
	// render-tree를 만지는 claude step에만 적용. strict면 키 자체를 넣지 않아
	// 기존 step 정의와 동일한 형태를 유지한다.
	const renderTreeConstraint: { constraint: StepConstraint } | Record<string, never> =
		options.enrichment ? { constraint: "free" } : {};
	...
	return definePipeline({
		...
		steps: [
			...
			// 02는 계획만 세우고 트리를 만들지 않으므로 항상 strict(catalogGaps 포착은
			// strict에서도 동작한다).
			defineStep({
...
/** 기본(strict) 파이프라인 — 기존 동작과 동일. */
export const screenGenerationPipelineV1 = buildScreenGenerationPipeline({
	id: "screen-generation",
	enrichment: false,
});

/** enriched 파이프라인 — render-tree step(04/06/09)이 catalogGap 기반 merge를 허용. */
export const screenGenerationEnrichedPipelineV1 = buildScreenGenerationPipeline({
	id: "screen-generation-enriched",
	enrichment: true,
});
