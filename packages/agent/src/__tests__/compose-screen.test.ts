...
describe("composeScreen", () => {
	it("프롬프트에 archetype catalog가 prior로 포함된다", async () => {
		const queryFn: LlmQueryFn = vi
			.fn()
			.mockResolvedValueOnce(makeQueryResultFromOutput(makeValidOutput()));

		await composeScreen({ prddScreenRecord: makePrdd(), ...makeDecks() }, { queryFn });

		const firstPrompt = (queryFn as ReturnType<typeof vi.fn>).mock.calls[0][0].prompt as string;
		expect(firstPrompt).toContain("Archetype Catalog");
		expect(firstPrompt).toContain("commerce-detail");
		expect(firstPrompt).toContain("list-browse");
