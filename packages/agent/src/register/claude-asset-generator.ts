...
function buildPrompt(input: ClaudeAssetGeneratorInput) {
	return [
		...
		"",
		"Routes & variants (one variant per main-flow screen file):",
		'- Register exactly one Screen Route covering all provided screens. route.id = importId.',
		'- A main-flow screen file ends with "-0" (e.g. NOVA-PRDD-PG-001-0.md). For each such file:',
		'    variant.id = main screen id with trailing "-0" stripped (e.g. NOVA-PRDD-PG-001)',
		'    variant.name        = main screen\'s "화면 명"',
		'    variant.description = main screen\'s "화면 설명"',
		'- Under each variant, register screens in this order:',
		"    1) the main screen (order 1) using the file's full 화면 ID (e.g. NOVA-PRDD-PG-001-0)",
		...
		'- Non "-0" files (e.g. -1, -2, -E1) are NOT separate variants. They are either',
		"  registered as edge screens of an existing variant via the main file's 케이스 분기 table,",
		"  or — if a -1/-2/etc. file represents an additional main-flow state — registered as an extra",
		"  screen under the matching variant with order following the main screen.",
		"",
