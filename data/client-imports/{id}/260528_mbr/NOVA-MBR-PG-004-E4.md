---
화면 ID: NOVA-MBR-PG-004-E4
화면 명: 회원 검증-가입제한 회원
화면 설명: 가입제한 회원 — 가입 불가 사유 안내.
화면 경로: 회원 가입 > 회원 검증
구현 유형: PG
...
| MemberVerifySection | 1 | CalloutGuardianConsentStatus | 가입제한 안내 | Callout | - | WithTitle | title.text: 가입이 제한된 고객입니다<br>body.text: 가입이 제한된 사유를 확인해 주세요. (예: {제한사유} = REJOIN_LIMITED) | - | - | - | PI-MBR-STAT-001-07, PI-MBR-JOIN-001-05, PI-MBR-JOIN-001-09, PI-MBR-JOIN-001-11, PI-MBR-ROUTE-001-05 | 분기 SB 강조 노출 |
...
| ActionButtonSection | 1 | ActionButtonGoCs | 고객센터 이동 CTA | ActionButton | - | TwoButton | main.text: 고객센터 문의<br>secondary.text: 홈으로 | onClick<br>onClick | systemCall<br>navigate | tel:114<br>home | PI-MBR-JOIN-001-09 | 후속 동선 CTA |

## 상태 변화

| 트리거 | 컴포넌트 명 | 속성 | 변경 값 | 관련 처리 |
| --- | --- | --- | --- | --- |
| 가입제한 회원 판정 시 | CalloutGuardianConsentStatus | title.text | 신규 가입이 가능한 고객입니다 → 가입이 제한된 고객입니다 | FN-MBR-JOIN-003.exceptions[4] |

## 화면 흐름

| 구분 | 조건 | 화면 ID | 화면 명 | 전달 데이터 | 후속 처리 | 관련 흐름 |
| --- | --- | --- | --- | --- | --- | --- |
| 화면 전환 | 홈으로 이동 선택 시 | home | 홈 | - | - | FN-MBR-JOIN-003.exceptions[4] |
