---
화면 ID: NOVA-MBR-PG-004-E6
화면 명: 회원 검증-상태 조회 시스템 오류
화면 설명: 상태 조회 시스템 오류 — 재시도 안내.
화면 경로: 회원 가입 > 회원 검증
구현 유형: PG
...
| MemberVerifySection | 1 | CalloutGuardianConsentStatus | 상태 조회 오류 안내 | Callout | - | WithTitle | title.text: 일시적인 오류가 발생했습니다<br>body.text: 회원 상태 조회에 실패했습니다. 잠시 후 다시 시도해 주세요. | - | - | - | PI-MBR-STAT-001-10, PI-MBR-JOIN-001-10, PI-MBR-ROUTE-001-06 | 분기 SB 강조 노출 |
...
| ActionButtonSection | 1 | ActionButtonRetry | 재시도 CTA | ActionButton | - | TwoButton | main.text: 다시 시도<br>secondary.text: 홈으로 | onClick<br>onClick | apiCall<br>navigate | -<br>home | PI-MBR-JOIN-001-10 | 후속 동선 CTA |

## 상태 변화

| 트리거 | 컴포넌트 명 | 속성 | 변경 값 | 관련 처리 |
| --- | --- | --- | --- | --- |
| 상태 조회 시스템 오류 발생 시 | CalloutGuardianConsentStatus | title.text | 신규 가입이 가능한 고객입니다 → 일시적인 오류가 발생했습니다 | FN-MBR-COM-001.exceptions[1] |

## 화면 흐름

| 구분 | 조건 | 화면 ID | 화면 명 | 전달 데이터 | 후속 처리 | 관련 흐름 |
| --- | --- | --- | --- | --- | --- | --- |
| 화면 전환 | 상태 조회 재시도 성공 시 | NOVA-MBR-PG-004-0 | 회원 검증 | 세션ID, 인증결과 | - | FN-MBR-COM-001.exceptions[1] |
