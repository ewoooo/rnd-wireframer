---
화면 ID: NOVA-MBR-PG-004-E3
화면 명: 회원 검증-기존 탈퇴 회원
화면 설명: 기존 탈퇴 회원 — 재가입 가능 여부 확인 안내.
화면 경로: 회원 가입 > 회원 검증
구현 유형: PG
...
| MemberVerifySection | 1 | CalloutGuardianConsentStatus | 탈퇴 회원 안내 | Callout | - | WithTitle | title.text: 과거 탈퇴 이력이 있습니다<br>body.text: 재가입 가능 여부를 확인합니다. (예: {회원상태} = 탈퇴) | - | - | - | PI-MBR-STAT-001-06, PI-MBR-JOIN-001-04, PI-MBR-ROUTE-001-04 | 분기 SB 강조 노출 |
...
| ActionButtonSection | 1 | ActionButtonGoRejoin | 재가입 가능 여부 확인 CTA | ActionButton | - | OneButton | main.text: 재가입 가능 여부 확인 | onClick | apiCall | - | PI-MBR-ROUTE-001-04 | onSuccess 시 가능 화면 또는 불가 안내로 이동 |

## 상태 변화

| 트리거 | 컴포넌트 명 | 속성 | 변경 값 | 관련 처리 |
| --- | --- | --- | --- | --- |
| 기존 탈퇴 회원 판정 시 | CalloutGuardianConsentStatus | title.text | 신규 가입이 가능한 고객입니다 → 과거 탈퇴 이력이 있습니다 | FN-MBR-JOIN-003.exceptions[3] |

## 화면 흐름

| 구분 | 조건 | 화면 ID | 화면 명 | 전달 데이터 | 후속 처리 | 관련 흐름 |
| --- | --- | --- | --- | --- | --- | --- |
| 화면 전환 | 재가입 가능 시 | NOVA-MBR-PG-005-0 | 가입 완료 | 세션ID, 회원상태(탈퇴) | - | FN-MBR-JOIN-003.exceptions[3] |
