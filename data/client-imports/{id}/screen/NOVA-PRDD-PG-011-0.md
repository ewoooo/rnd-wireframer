---
화면 ID: NOVA-PRDD-PG-011-0
화면 명: 담기 전 유효성 재검증
화면 설명: 담기 실행 직전 상품 상태, 재고, 판매 기간, 채널 판매 가능 여부, 조합 정책을 다시 확인한다.
화면 경로: 담기 전 유효성 재검증
구현 유형: PG
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 화면 진입 정상 | 재검증 결과·재고·조합 검증 영역 기본 표시 | apiCall |
| loading | API 호출 | skeleton 표시 | - |
| error | 최종 재검증 중 가격·혜택·재고 변경 발생 | [영역 1] Callout 변경 비교 안내 노출 | apiCall |
| empty | 옵션 재고 결과 없음 | [영역 2] 빈 상태 안내 표시 | - |
| blocked | 재고·판매 상태·가입 조건 미확정 | [영역 1] CTA disabled + [영역 3] 보완 항목 강조 | setState |
| OneButton | 재검증 결과 정상 확정 | ActionButton main 슬롯만 활성 | - |
...
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-012-0 | 담기 실행과 상태 저장 | 담기 전 유효성 재검증 결과 확정 시 | 선택 스냅샷, 판정 근거, 기준 시각 | - |
| 케이스 분기 | NOVA-PRDD-PG-011-1 | 담기 전 유효성 재검증-가격·혜택·재고 변경 | 최종 재검증 중 가격·혜택·재고 변경 | - | 변경 내용 비교 표시 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-011-2 | 담기 전 유효성 재검증-동일 구성 중복 담기 | 동일 구성이 이미 담겨 있음 | - | 기존 담기 항목 갱신·이동 경로 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-011-E1 | 담기 전 유효성 재검증-장바구니 전환 실패 | 담기 저장 성공 후 장바구니 전환 실패 | - | 담기 항목 유지 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-011-3 | 담기 전 유효성 재검증-재고·판매 상태·가입 조건 미확정 | 재고·판매 상태·가입 조건 미확정 | - | 보완 가능 항목 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-011-4 | 담기 전 유효성 재검증-선택 조합 충돌 | 선택 조합 충돌 발생 | - | 충돌 항목만 수정 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-011-5 | 담기 전 유효성 재검증-로그인·인증 후 선택 구성 소실 | 로그인·인증 후 선택 구성 소실 | - | 선택 구성 복원 후 화면 유지 |
