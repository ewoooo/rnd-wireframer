---
화면 ID: NOVA-PRDD-PG-015-0
화면 명: 선택 불가·충돌 사유 안내
화면 설명: 선택 불가, 중복 가입, 조합 충돌, 상품 정책 제한 사유와 해결 방향을 안내한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|---|---|---|---|
| default | 화면 진입 정상 | 실패 사유 안내와 검증 결과 기본 표시 | apiCall |
| loading | API 호출 | skeleton 표시 | - |
| error | 동일 실패 반복 | [영역 1] Callout 오류 안내 + 대체 경로 강조 | apiCall |
| empty | 검증 대상 그룹상품 없음 | [영역 2] 검증 결과 항목 미노출 | - |
| blocked | 재고·판매·가입 조건 미확정 | [영역 1] 보완 항목 강조 + CTA disabled | setState |
...
| 999 | 1 | ActionButtonRecover | 화면 하단 복구 진행 CTA | ActionButton | default | onClick | navigate | NOVA-PRDD-PU-016-0 | main: 로그인·인증하고 계속하기 | 원위치 복귀 정보 (api:FN-PRDD-FAIL-001) | [정책:PI-PRDD-FAIL-001-03] 인증 복귀 |

## 화면 흐름

| 구분 | 화면 ID | 화면 명 | 조건 | 전달 데이터 | 후속 처리 |
|---|---|---|---|---|---|
| 화면 전환 | NOVA-PRDD-PU-016-0 | 로그인·인증 후 원위치 복귀 | 선택 불가·충돌 사유 안내 결과 확정 시 | 원위치 복귀 토큰, 이전 선택 구성, 실패 사유 | - |
| 케이스 분기 | NOVA-PRDD-PG-015-E1 | 선택 불가·충돌 사유 안내-동일 실패 반복 | 동일 실패 반복 | - | 대체 상품·상담·나중에 다시 시도 경로 제시 |
| 케이스 분기 | NOVA-PRDD-PG-015-E2 | 선택 불가·충돌 사유 안내-상담 전환 문맥 누락 | 상담 전환 문맥 누락 | - | 상품 ID·옵션·오류 사유 보완 후 상담 접수 |
| 케이스 분기 | NOVA-PRDD-PG-015-E3 | 선택 불가·충돌 사유 안내-인증 실패 후 원위치 복귀 불가 | 인증 실패 후 원위치 복귀 불가 | - | 다시 선택해야 하는 항목 명확히 안내 |
| 케이스 분기 | NOVA-PRDD-PG-015-E4 | 선택 불가·충돌 사유 안내-재고·판매·가입 조건 미확정 | 재고·판매·가입 조건 미확정 | - | 담기 미확정 후 보완 가능 항목 안내 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-015-1 | 선택 불가·충돌 사유 안내-선택 조합 충돌 | 선택 조합 충돌 | - | 충돌 항목만 수정하도록 안내 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-015-2 | 선택 불가·충돌 사유 안내-로그인·인증 후 복귀 시 선택 구성 소실 | 로그인·인증 후 복귀 시 선택 구성 소실 | - | 재선택 없이 복원 가능한 상태로 전환 |
