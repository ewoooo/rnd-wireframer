---
화면 ID: NOVA-PRDD-PG-013-0
화면 명: 담기 완료 후 다음 행동 선택
화면 설명: 고객이 계속 탐색, 장바구니 이동, 바로 신청, 비교 계속하기 중 다음 행동을 선택한다.
...
| 상태 | 트리거 | 컴포넌트 변화 | 액션 |
|------|--------|---------------|------|
| default | 화면 진입 정상 | 다음 행동 선택지·CTA 안내 영역 기본 표시 | apiCall |
| loading | API 호출 | skeleton 표시 | - |
| error | 미디어·후기·스펙 로딩 실패 | [영역 3] 핵심 요약 유지 + 일부 항목 미노출 | apiCall |
| empty | 상품 기준 정보 누락 | [영역 3] 보완 필요·상담 가능 경로 안내 | - |
| blocked | 로그인·인증 후 선택 구성 소실 | [영역 2] 선택 구성 복원 안내 표시 | apiCall |
| TwoButton | 다음 행동 선택지 표시 | ActionButton main·secondary 슬롯 활성 | - |
...
|------|---------|---------|------|-------------|-----------|
| 화면 전환 | NOVA-PRDD-PG-014-0 | 장바구니·주문 진입 연결 | 다음 행동(장바구니 이동·바로 신청) 선택 시 | 담기 ID, 선택 스냅샷, 전환값 | - |
| 케이스 분기 | NOVA-PRDD-PG-013-1 | 담기 완료 후 다음 행동 선택-재고·판매 상태·가입 조건 미확정 | 재고·판매 상태·가입 조건 미확정 | - | 보완 가능 항목 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-013-2 | 담기 완료 후 다음 행동 선택-선택 조합 충돌 | 선택 조합 충돌 발생 | - | 충돌 항목만 수정 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-013-3 | 담기 완료 후 다음 행동 선택-로그인·인증 후 선택 구성 소실 | 로그인·인증 후 선택 구성 소실 | - | 선택 구성 복원 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-013-4 | 담기 완료 후 다음 행동 선택-상품 기준 정보 누락 | 상품 기준 정보 누락 | - | 보완 필요·상담 가능 경로 안내 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-013-E1 | 담기 완료 후 다음 행동 선택-내부 운영 코드 노출 | 내부 운영 코드·원장 필드명 노출 감지 | - | 배포 제한 후 화면 유지 |
| 케이스 분기 | NOVA-PRDD-PG-013-E2 | 담기 완료 후 다음 행동 선택-미디어·후기·스펙 로딩 실패 | 미디어·후기·스펙 로딩 실패 | - | 핵심 요약·가격·조건 판단 유지 후 화면 유지 |
