...
| `GET` | `/api/inference/runs` | jobId 기준 inference run 목록 조회 | 없음 | `{ runs }` |
...
| `POST` | `/api/inference/:jobId/rerun` | job 재실행. terminal(succeeded/failed) job만 허용 — 진행 중이면 `409`. `startFromStepId` 이전 step은 스킵(모르는 step id는 `400`), 실행 범위 step snapshot은 `pending`으로 리셋. `contextOverrides`는 실행 전 working memory에 기록 (key는 `^[a-z0-9-]+$`, 위반 시 `400`) | `jobId` path param, optional JSON `{ startFromStepId?, contextOverrides? }` | `202` `{ job, ok, startFromStepId, overriddenContextKeys? }` |
...
- Browser-facing hook은 `apps/web/src/feature/<domain>/api/*` 또는 기존 `lib/*` client helper를 통해 `/api/*`를 호출하고, `server/*`를 직접 import하지 않는다.
