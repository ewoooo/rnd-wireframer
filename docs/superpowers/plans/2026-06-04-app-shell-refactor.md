...
> ⚠️ **정정(실행 중 발견):** 이 프로젝트는 `scripts/check-react-hooks-policy.mjs`로 `useMemo`/`useCallback`을 **전면 금지**한다. 따라서 아래 Task 4의 "useMemo로 감싼다" 지침은 무효이며, 파생상태는 plain const로 그대로 둔다(원본과 동일). 이것이 정책 준수이자 순수 이동에도 부합한다.
