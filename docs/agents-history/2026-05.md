...
- 변경: `@cx/agent/promote-database-tables`를 추가해 `*.db-tables.json` 후보의 shape, route/variant/screen/area/component 참조 무결성, renderer projection validation을 검사한 뒤 `database/tables/*.json` wrapper payload를 만들도록 구현함
- 변경: `/api/agent/promote-ai-import`와 `apps/web/src/server/agent/promote-ai-import.ts`를 추가해 `database/ai-imports/{candidate}.db-tables.json` 후보를 dry-run 기본값으로 검증하고, 명시적으로 `dryRun: false`일 때만 `database/tables`에 반영하도록 연결함
...
- 변경: `apps/web` generate route가 Claude 실행 전에 `client-import.parsed.json`, `client-import.validation.json`, `client-import.db-tables.json` 후보 산출물을 `database/ai-imports`에 먼저 기록하도록 연결함
...
- 후속: `client-import.db-tables.json` 또는 `agent-assets.db-tables.json`을 검증 후 `database/tables`로 반영하는 promote/import adapter 구현
...
- 변경: `generate-register`가 AI 생성 결과를 `database/tables`에 직접 쓰지 않고 `database/ai-imports/agent-assets.db-tables.json` 후보 산출물로만 남기도록 변경함
...
- 후속: `database/ai-imports/*.db-tables.json` 후보를 `database/tables`로 반영하는 promote/import adapter와 검증 리포트 추가 필요
