...
`scripts/smoke-pipeline.ts`는 위 흐름을 개발자가 반복 실행하는 CLI 진입점이다.
...
design skill과 design-context bundle의 에이전트용 규칙 정본은 `packages/agent/docs/` 아래에 둔다. `@cx/inference-nodes`는 내부 planning helper를 통해 skill/bundle ref를 선택하고, `@cx/pipeline`은 bundle 본문 IO를 수행해 agent stage context에 주입한다.
...
| `@cx/inference-nodes` | pipeline에서 실행하는 inference 작업 단위와 내부 planning helper | screen-generation agent/deterministic/validation node wrapper, SourceSpec -> pattern layer candidates, agent input 조립, 후속 action helper, pipeline-provided runner 실행, node별 input -> output 결과 반환 | stage 순서 소유, persistence, 파일 IO, Claude runner 구현, artifact write, React render |
...
- `@cx/adapters/markdown`은 이미 읽힌 Markdown/source bundle을 받아 SourceSpec과 parser issue를 반환한다.
...
- smoke scripts는 `@cx/pipeline`만 호출하고 각 패키지의 규칙을 재구현하지 않는다.
...
`@cx/inference-nodes`는 inference node 내부에서 쓰는 deterministic helper와 실행 가능한 node wrapper를 함께 제공한다. SourceSpec, 후보 pattern, validation report 같은 입력을 agent task input이나 next-action data로 조립하지만 실행 가능한 pipeline plan을 소유하지 않는다. `@cx/pipeline`은 `@cx/inference-nodes`의 실행 가능한 node wrapper만 호출한다.

`scripts/smoke-pipeline.ts`는 개발 환경에서 `runPipeline("screen-generation", options)`만 호출하는 thin harness다. smoke scripts는 `@cx/inference-nodes`, `@cx/agent`, `@cx/validation`, catalog 패키지를 직접 import하지 않는다.
...
```text
script
-> scripts/generation runGenerationSmoke
-> @cx/pipeline runPipeline("screen-generation")
-> PIPELINE_STAGE_PROTOCOL.md의 screen-generation stage sequence
```

root script는 `scripts/smoke-pipeline.ts` CLI를 호출한다. 이 helper는 public package가 아니라 개발 스크립트 전용이며, 내부 구현은 `@cx/pipeline`의 `runPipeline("screen-generation", options)`를 호출한다.
