...
import { GripVertical, Loader2, Workflow } from "lucide-react";
...
import { AgentRegistryInspection } from "./agent/AgentRegistryInspection";

export function WorkBenchInspectionPanel() {
...
function ConnectedOrganismList({
	...
	const canReorder = screenOrganisms.length > 1 && !saving;

	async function persistOrder(nextOrganismCodes: string[], previousOrganismCodes: string[]) {
		setSaving(true);
		setSaveError("");

		try {
			const response = await fetch(
				`/api/sample-screens/${encodeURIComponent(screenCode)}/organism-order`,
				{
					method: "PUT",
					headers: {
					...
					body: JSON.stringify({
						organismCodes: nextOrganismCodes,
					}),
				},
			);
			...
			if (!response.ok) {
				...
				throw new Error(payload.error ?? "Failed to update screen organism order");
			}
		} catch (error) {
			onReorder(screenCode, previousOrganismCodes);
	...
	function handleDrop(targetOrganismCode: string) {
		if (!draggedOrganismCode || draggedOrganismCode === targetOrganismCode || saving) {
		...
		setDraggedOrganismCode("");
		void persistOrder(nextOrganismCodes, previousOrganismCodes);
	}
		...
		<div className="flex flex-col gap-2">
			<div className="flex items-center justify-between gap-3">
				...
				{saving ? (
					...
					<Badge variant="outline">screen.json</Badge>
				)}
			</div>
