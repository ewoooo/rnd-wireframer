...
export function ExportToolbar() {
	...
	// Export only makes sense at the screen level. Area/component (and agent) views can't
	// be exported yet → disable the buttons there.
	const canExport = !!selectedScreen && !isComponentView && !isAreaView;
	const disabledReason =
		isComponentView || isAreaView ? "스크린에서만 내보낼 수 있어요" : "스크린을 선택해주세요";
	...
	function buildContent(kind: ExportKind): string {
		...
		return kind === "json"
			? renderTreeToJson(selectedScreen.schema)
			: renderTreeToBuildCode(selectedScreen.schema);
	}
	...
	return (
		<TooltipProvider>
			...
			{open && selectedScreen ? (
				<CodeModal
					title={`${TITLE[open]} · ${selectedScreen.code}`}
