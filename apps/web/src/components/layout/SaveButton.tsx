...
import { updateAreaChildren } from "@/app/actions/area-actions";
import { updateScreenRegions } from "@/app/actions/screen-actions";
import { Button } from "@/components/ui/button";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { areaNodeToChildren, renderTreeToTables } from "@/adapters/render-tree-to-tables";
...
export function SaveButton() {
	...
	// Screen(scn)·Area(ogn) 페이지에 저장 백엔드 연결됨. Component(comp)는 추후.
	const canSave =
		(activeTab === "scn" && !!selectedScreen) || (activeTab === "ogn" && !!selectedArea);

	function handleSave() {
		startSaving(async () => {
			let error: string | undefined;
			if (activeTab === "ogn" && selectedArea) {
				const children = areaNodeToChildren(selectedArea.node);
				({ error } = await updateAreaChildren(selectedArea.code, children));
				...
				({ error } = await updateScreenRegions(selectedScreen.code, screen));
			} else {
				return;
			}
			if (!error) {
